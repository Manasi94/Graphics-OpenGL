...
// Plot points using circle's 8 way symmetry
...
{ 
    ...
    int fl = 0;//flag used for the breaking point condition
	...
	for(i=0;i<n;i++)
	{	
		printf("enter details of %dth node:father, left son, right son, height ",i);
		scanf("%d %d %d %d",&t[i].father,&t[i].left_son,&t[i].right_son,&t[i].height);
		if(t[i].left_son == 1)
		{
            printf("Enter left son index: ");
            scanf("%d",(&t[i].left_son_index));
        }
        if(t[i].right_son == 1)
        {
        	printf("Enter right son index: ");
    ...
    for(i=0;i<n;i++)
    {
        ...
        if(t[i].left_son)
        {
            ...
            line(t[i].x*30+200,640-t[i].y*30,t[t[i].left_son_index].x*30+200,640-t[t[i].left_son_index].y*30);
            sleep(2);
        }
        if(t[i].right_son)
        {
            printf("Draw Line bw : father %d %d and right son %d %d\n",t[i].x,t[i].y,t[t[i].right_son_index].x,t[t[i].right_son_index].y);
            line(t[i].x*30+200,640-t[i].y*30,t[t[i].right_son_index].x*30+200,640-t[t[i].right_son_index].y*30);
            sleep(2);
        }
    }
	glFlush();
